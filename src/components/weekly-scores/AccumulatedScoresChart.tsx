...
export const AccumulatedScoresChart: FunctionComponent<
	AccumulatedScoresChartProps
> = ({ data }) => {
	const pointsToShow = 4;
	...
	const labels = [
		...Object.keys(data)
			.sort(
				(weekA, weekB) => new Date(weekA).getTime() - new Date(weekB).getTime(),
			)
			.slice(-pointsToShow),
	];

	const accumulated: ({ week: string } & Points)[] = values.reduce(
		(acc, curr, index) => {
	...
	const chartData = {
		...
		datasets: [
			{
				label: "Eric",
				data: accumulated.slice(-pointsToShow).map((d) => d.eric),
			...
			{
				label: "Niels",
				data: accumulated.slice(-pointsToShow).map((d) => d.niels),
			...
			{
				label: "Marcel",
				data: accumulated.slice(-pointsToShow).map((d) => d.marcel),
