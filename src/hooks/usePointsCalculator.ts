...
export const usePointsCalculator = () => {
	...
	function addPointsToWeeklyGuess(row: WeeklyGuess): ScoredWeeklyGuess {
		if (row.actual === "-" || row.actual === "WFH.") {
			return row;
		}

		const points = Object.fromEntries(
			Object.entries(row.guesses).map(([player, guess]) => [
				player,
				calculateDifference(guess, row.actual),
			]),
		);
	...
	function getPlayerStats(allTimeData: Record<string, Points>) {
		...
		const baseStats = Object.fromEntries(
			players.map((player: string) => {
				...
				return [player, { total, min, avg }];
			}),
		) as Record<
			(typeof players)[number],
			{ total: number; min: number; avg: number }
		>;
